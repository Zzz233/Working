...
class Pubmed:
    ...
    def run(self):
        ...
        for path in self.get_path():
            ...
            #     break
            time_start = time.time()
            session.bulk_save_objects(detail_data)
            session.bulk_save_objects(author_data)
            session.bulk_save_objects(keyword_data)
            session.bulk_save_objects(grants_data)
            session.bulk_save_objects(p_type_data)
            session.bulk_save_objects(corrections_data)
            # session.bulk_save_objects(reference)
            try:
                session.commit()
                print("done")
