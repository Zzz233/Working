...
class Pubmed:
    ...
    def delete_items(self, duplicated_list):
        with self.session_maker() as db_session:
            for item in duplicated_list:
                ...
                db_session.query(Keyword).filter(Keyword.Article_pmid == item).delete()
                db_session.query(Info).filter(Info.Article_pmid == item).delete()
                db_session.query(Grant).filter(Grant.Article_pmid == item).delete()
                db_session.query(Publication).filter(Publication.Article_pmid == item).delete()
                db_session.query(Corrections).filter(Corrections.Article_pmid == item).delete()
        print('删除完成')

    def convert2ori(self, detail, author, keyword, grants, p_type, corrections):  # 分类 接收原有对象 重新生成各个表的对象
    ...
    def add_data(self, detail_ori, author_ori, keyword_ori, grants_ori, p_type_ori, corrections_ori):  #
        with self.session_maker() as db_session:
