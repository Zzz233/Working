...
class Detail(Base):
    __tablename__ = "abbkine_kit_detail"
...
class Citations(Base):
    __tablename__ = "abbkine_kit_citations"
...
class Images(Base):
    __tablename__ = "abbkine_kit_images"
...
class Price(Base):
    __tablename__ = "abbkine_kit_price"
...
class Abbkine(object):
    ...
    def format(self, url):
        with requests.Session() as s:
            ...
            content = html_lxml.xpath('//section[@class="l-product"]')[0]
        return content
    ...
    def catalog_number(self, html):
        mora = html.xpath('.//h4[contains(text(), "Cat #: ")]/text()')[0].replace(
            "Cat #: ", ""
        )
        return mora
    ...
    def product_name(self, html):
        mora = html.xpath(
            './/span[@class="info-name"][contains(text(), "Product name")]/following-sibling::p/text()'
        )[0].strip()
        return mora
    ...
    def sensitivity(self, html):
        try:
            mora = html.xpath(
                './/span[@class="info-name"][contains(text(), "Limit of detection")]/following-sibling::p/text()'
            )[0].strip()
            return mora
        except Exception:
            return None
    ...
    def assay_range(self, html):
        try:
            mora = html.xpath(
                './/span[@class="info-name"][contains(text(), "Calibration range")]/following-sibling::p/text()'
            )[0].strip()
            return mora
        except Exception:
            return None
    ...
    def geneid(self, html):
        try:
            mora = html.xpath(
                './/span[@class="info-name"][contains(text(), "Gene ID")]/following-sibling::p/a/text()'
            )[0].strip()
            return mora
        except Exception:
            return None
    ...
    def swissprot(self, html):
        try:
            mora = html.xpath(
                './/span[@class="info-name"][contains(text(), "Accession")]/following-sibling::p/a/text()'
            )[0].strip()
            return mora
        except Exception:
            return None
    ...
    def datasheet_url(self, html):
        try:
            mora = html.xpath('.//a[@title="Datasheet"]/@href')[0].strip()
            return mora
        except Exception:
            return None
    ...
    def review(self, html):
        try:
            mora = int(
                html.xpath('.//a[@href="#reviews"]/text()')[0]
    ...
    def image_qty(self, html):
        try:
            mora = len(
                html.xpath(
                    './/div[@class="product-details"]/div[@class="det-btm"]/a/img[not(@src="https://www.abbkine.com/file/img/elisa-kit.jpg")]'
                )
            )
            return mora
        except Exception:
    ...
    def citations(self, html):
        try:
            mora = int(
                html.xpath('.//a[@href="#tab"]/text()')[0]
                .replace("Product Publications(", "")
                .replace(")", "")
            )
            return mora
        except Exception:
    ...
    def sub_images(self, image_qty, html):
        ...
        if image_qty == 0:
            ...
            for item in imgs:
                try:
                    img_url = item.xpath("./@src")[0].strip()
                    try:
                        img_des = item.xpath("./../../p/text()")[0].strip()
                    except Exception:
                        img_des = None
...
if __name__ == "__main__":
    # for i in range(1):
    while r.exists("abbkine_kit_detail"):
        extract = r.rpop("abbkine_kit_detail")
        # extract = (
        #     "https://www.abbkine.com/product/elikine-human-ccl2-elisa-kit-ket6001/"
        # )
        print(extract)
        try:
            lxml = Abbkine().format(extract)
        except Exception as e:
            print(e)
            r.lpush("abbkine_kit_detail", extract)
            print("sleeping...")
            time.sleep(30)
            continue
        if lxml is not None:
            brand = Abbkine().brand()
            kit_type = Abbkine().kit_type()
            catalog_number = Abbkine().catalog_number(lxml)
            product_name = Abbkine().product_name(lxml)
            detail_url = Abbkine().detail_url(extract)
            # tests = Abbkine().tests(lxml)
            assay_type = Abbkine().assay_type(lxml)
            detection_method = Abbkine().detection_method(lxml)
            sample_type = Abbkine().sample_type(lxml)
            assay_length = Abbkine().assay_length(lxml)
            sensitivity = Abbkine().sensitivity(lxml)
            assay_range = Abbkine().assay_range(lxml)
            # specificity = Abbkine().specificity(lxml)
            # target_protein = Abbkine().target_protein(lxml)
            geneid = Abbkine().geneid(lxml)
            swissprot = Abbkine().swissprot(lxml)
            datasheet_url = Abbkine().datasheet_url(lxml)
            review = Abbkine().review(lxml)
            image_qty = Abbkine().image_qty(lxml)
            citations = Abbkine().citations(lxml)
            synonyms = Abbkine().synonyms(lxml)
            # conjugate = Abbkine().conjugate(lxml)
            species_reactivity = Abbkine().species_reactivity(lxml)
            # note = Abbkine().note(lxml)

            # sub_citations = Abbkine().sub_citations(citations, lxml)
            sub_images = Abbkine().sub_images(image_qty, lxml)
            sub_price = Abbkine().sub_price(lxml)
            # print(sub_price)
