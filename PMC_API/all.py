...
pool = redis.ConnectionPool(host="192.168.124.9", port=6379, decode_responses=True, db=14)
...
class API:
    ...
    def get_content(self, pmc, proxies):
        resp = requests.get(self.base_url + pmc, timeout=45, headers=self.headers, proxies=proxies)
    ...
    def judge(self, xml, pmid, pmcid, issn, journal_name, article_type):
        ...
        if xml.xpath('//h1[@class="content-title"]/text()'):
            title_list = xml.xpath('//h1[@class="content-title"]//text()')
            title = ' '.join(t.strip() for t in title_list)
        else:
            title = 0
        # print('title', title)
        ...
        if xml.xpath('//h2[contains(text(), "Result")]/..//p'):
            results_list = []
            results = xml.xpath('//h2[contains(text(), "Result")]/..//p')
            ...
            for result in results:
                ...
                if len(r_str) > 100:
                    results_list.append(r_str)
        elif xml.xpath('//h2[contains(text(), "result")]/..//p'):
            results_list = []
            results = xml.xpath('//h2[contains(text(), "result")]/..//p')
            for result in results:
                r_text = result.xpath('.//text()')
        ...
        if xml.xpath('//h2[contains(text(), "Acknowledgement")]/..//p'):
            acks_list = []
            acks = xml.xpath('//h2[contains(text(), "Acknowledgement")]/..//p')
            for ack in acks:
                ack_text = ack.xpath('.//text()')
                ack_str = ''.join(aa.strip() for aa in ack_text)
                if len(ack_str) > 100:
                    acks_list.append(ack_str)
        elif xml.xpath('//h2[contains(text(), "ACKNOWLEDGEMENT")]/..//p'):
            acks_list = []
            acks = xml.xpath('//h2[contains(text(), "ACKNOWLEDGEMENT")]/..//p')
            for ack in acks:
                ack_text = ack.xpath('.//text()')
                ...
                if len(ack_str) > 100:
                    acks_list.append(ack_str)
        elif xml.xpath('//h2[contains(text(), "Acknowledgment")]/..//p'):
            acks_list = []
    ...
    def run(self):
        ...
        proxies = {
            "http": "http://" + '114.104.183.84:4278',
            "https": "http://" + '114.104.183.84:4278',
        }
        for task in self.get_task():
            ...
            try:
                resp, xml = self.get_content(pmcid, proxies)
