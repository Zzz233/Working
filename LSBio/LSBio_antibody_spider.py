...
class LSbio():
    ...
    def sellable(self, html):
        if html.xpath(
                './/div[@class="c-OrderBox__OrderForm"]'):
            sellable = 'yes'
        else:
            sellable = 'no'
        return sellable
    ...
    def modified(self, html):
        if html.xpath(
            ...
            modified = html.xpath(
                ...
                '::div[@class="DataText"]/text()')[0].strip()
        else:
            modified = None
        return modified
    ...
    def swissprot(self, html):
        if html.xpath(
                './/label[contains(text(), "Uniprot:")]'):
            swissprot = html.xpath(
                './/label[contains(text(), "Uniprot:")]/'
                'following-sibling::a/text()')[0].strip()
        else:
            swissprot = None
        return swissprot
    ...
    def citations(self, html):
        if html.xpath(
            ...
            citations = html.xpath(
                './/h2[@class="c-DetailPage__MainContentSectionHeaderText"]'
                '[contains(text(), "Publications (")]/text()')[0].strip()
    ...
    def review(self, html):
        review = html.xpath(
            './/h2[@class="c-DetailPage__MainContentSectionHeaderText"]'
            '[contains(text(), "Customer Reviews")]/text()')[0].strip()
    ...
    def sub_application(self, html):
        result = []
        if html.xpath(
                './/ul[@class="l-AntibodySpecifications__AssayList"]'):
            ul = html.xpath(
                './/ul[@class="l-AntibodySpecifications__AssayList"]')[0]
        else:
            return result
        result = []
    ...
    def sub_citations(self, html):
        ...
        for item in sub_citations:
            article_title = item.xpath('.//div[@class="l-Publi'
                                       'cation__Text"]/text()')[0].strip()
            pmid = item.xpath('.//a[contains(text()'
                              ', "PubMed:")]/text()')[0].split(': ')[1]
            pubmed_url = item.xpath('.//a[contains(text(),'
                                    ' "PubMed:")]/@href')[0]
        ...
        for item in sub_images:
            if 'Height' in item:
                image_url = item.xpath(
                    ...
                    'ch_Height"]/@src')[0].strip()
                image_decription = item.xpath(
                    './/img[@class="c-ImageCarousel__Image '
                    'c-ImageCarousel__Image-Type_Primary c-I'
                    'mageCarousel__Image-Type_Primary-Stret'
                    'ch_Height"]/@alt')[0].strip()
                ...
                image_url = item.xpath(
                    ...
                    'c-ImageCarousel__Image-Type_Primary c-I'
                    'mageCarousel__Image-Type_Primary-Stret'
                    'ch_Width"]/@src')[0].strip()
                image_decription = item.xpath(
                    './/img[@class="c-ImageCarousel__Image '
                    'c-ImageCarousel__Image-Type_Primary c-I'
                    'mageCarousel__Image-Type_Primary-Stret'
                    'ch_Width"]/@alt')[0].strip()
                result.append([image_url, image_decription])
    ...
    def sub_price(self, html):
        result = []
        if html.xpath(
                './/div[@class="l-OrderBox__ProductsTableRow"]'):
            orders = html.xpath(
                './/div[@class="l-OrderBox__ProductsTableRow"]')
        else:
            return result
        for i, item in enumerate(orders):
            sub_catalog_number = item.xpath(
                './/div[@class="c-OrderBox__PricingTable'
                'CatalogCol"]/text()')[0].strip()
            sub_price = item.xpath('.//input/@value')[0]
            sub_size = html.xpath(
                './/select[@class="c-OrderBox__SizeSelect"]/option/text()')[i]
