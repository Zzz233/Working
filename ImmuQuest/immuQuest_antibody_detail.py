...
class ImmuQuest():
    ...
    def clone_number(self, name):
        if '[' in name:
            clone_number = name.split('[')[1].split(']')[0].replace(
                ' Polyclonal', '')
        else:
            clone_number = None
        return clone_number
    ...
    def citations(self, html):
        try:
            citations = len(
                html.xpath('.//td[@id="pubmed_IDs"]/text()')[0].split(
                    ','))
        except Exception as e:
            citations = None
        return citations
    ...
    def image_qty(self, html):
        try:
            image_qty = len(html.xpath(
                './/ul[@id="product-thumbnails"][@class="slides"]/li'))
        except Exception as e:
            image_qty = None
        return image_qty
    ...
    def sub_citations(self, html):
        ...
        try:
            ...
            for item in sub_cite:
                sub_title = item.xpath('./text()')[
                    0].strip()
                sub_url = item.xpath('./a/@href')[
                    0].strip()
                if 'www.ncbi.nlm.nih.gov/pubmed/' in sub_url and 'www.ncbi.nlm.nih.gov/pubmed/?' not in sub_url:
                    sub_pmid = sub_url.split('www.ncbi.nlm.nih.gov/pubmed/')[
                        1].replace('?dopt=Abstract', '')
                elif 'pubmed.ncbi.nlm.nih.gov/' in sub_url:
                    sub_pmid = sub_url.split('pubmed.ncbi.nlm.nih.gov/')[
                        1]
                else:
                    sub_pmid = None
                if sub_title == 'Test':
