...
class Data(Base):
    __tablename__ = "biovision_kit_list"
...
for i in range(1):
    url = "https://www.biovision.com/products/elisa-kits/elisa-kits-a-z.html?limit=all"
    with requests.Session() as s:
        resp = s.get(url=url, headers=headers)
        lxml = etree.HTML(resp.text)
        divs = lxml.xpath('//div[@class="product-shop-inner"]')
        # print(len(divs))
        for item in divs:
            link = item.xpath('./h2[@class="product-name"]/a/@href')[0].strip()
            name = item.xpath('./h2[@class="product-name"]/a/@title')[0].strip()
