...
headers = {
    ...
    "Referer": "https://www.abbexa.com/products/elisa-kits?limit=100",
...
brand = "abbexa"
for i in range(305, 582):
    url = f"https://www.abbexa.com/products/elisa-kits?limit=100&page={i}"
    with requests.Session() as s:
        resp = s.get(url=url, headers=headers)
        lxml = etree.HTML(resp.text)
        divs = lxml.xpath('//div[@class="product_holder_inside"]')
        for div in divs:
            link = div.xpath('.//a[@style="font-weight: bold;"]/@href')[0].strip()
            name = div.xpath('.//a[@style="font-weight: bold;"]/text()')[0].strip()
            catano = div.xpath('.//span[@style="font-size:12px;"]/text()')[0].strip()
