...
class Detail(Base):
    __tablename__ = "neobiolab_kit_detail"
...
class Citations(Base):
    __tablename__ = "neobiolab_kit_citations"
...
class Images(Base):
    __tablename__ = "neobiolab_kit_images"
...
class Price(Base):
    __tablename__ = "neobiolab_kit_price"
...
class Neobiolab(object):
    ...
    def format(self, url):
        with requests.Session() as s:
            ...
            content = html_lxml.xpath('//div[@class="human-kits-details cf"]')[0]
        return content
    ...
    def catalog_number(self, html):
        try:
            mora = html.xpath(
                './/div[@class="row"]/div[@class][contains(text(), "Catalog #")]/following-sibling::div[@class]/text()'
            )[0].strip()
            return mora
        except Exception:
            return None
    ...
    def product_name(self, html):
        try:
            mora = html.xpath(".//h3/text()")[0].strip()
            return mora
        except Exception:
            return None
    ...
    def tests(self, html):  # 48T/96T 使用次数 规格
        try:
            mora = html.xpath(
                './/div[@class="row"]/div[@class][contains(text(), "Size")]/following-sibling::div[@class]/text()'
            )[0].strip()
            return mora
        except Exception:
            return None
    ...
    def assay_type(self, html):
        try:
            mora = html.xpath(
                './/div[@class="row"]/div[@class][contains(text(), "Precautions")]/following-sibling::div[@class]/text()'
            )[0].strip()
            return mora
        except Exception:
            return None
    ...
    def detection_method(self, html):
        try:
            mora = html.xpath(
                './/div[@class="row"]/div[@class][contains(text(), "Detection")]/following-sibling::div[@class]/text()'
            )[0].strip()
            return mora
        except Exception:
            return None
    ...
    def sample_type(self, html):
        try:
            mora = html.xpath(
                './/div[@class="row"]/div[@class][contains(text(), "Sample")]/following-sibling::div[@class]/text()'
            )[0].strip()
            return mora
        except Exception:
            return None
    ...
    def assay_length(self, html):
        try:
            mora = html.xpath(
                './/div[@class="row"]/div[@class][contains(text(), "Assay Time")]/following-sibling::div[@class]/text()'
            )[0].strip()
            return mora
        except Exception:
            return None
    ...
    def datasheet_url(self, html):
        try:
            mora = ('https://neobiolab.com' 
                + html.xpath('.//a[@target="_blank"][contains(text(), "Datasheet ")]/@href')[0].strip())
            return mora
        except Exception:
            return None
    ...
    def image_qty(self, html):
        try:
            mora = len(html.xpath('.//div[@class="graph-img"]/img'))
            return mora
        except Exception:
            return 0
    ...
    def species_reactivity(self, html):
        try:
            mora = html.xpath(
                './/td[@class="hkb-title"][contains(text(), "CROSS REACTS WITH")]/following-sibling::td[@class="hkb-value"]/text()'
            )[0].strip()
            return mora
        except Exception:
            return None
    ...
    def sub_images(self, image_qty, html):
        ...
        if image_qty == 0:
            ...
            imgs = html.xpath('.//div[@class="graph-img"]/img')
            for img in imgs:
                try:
                    img_url = (
                        "https://neobiolab.com"
                        + img.xpath("./@src")[0].strip()
                    )
    ...
    def sub_price(self, html):
        results = []
        try:
            divs = html.xpath('.//div[@class="hkdr-top"]/div[@class="row"]')
        except Exception:
            return results
        for item in divs:
            sub_siz = item.xpath(".//option[@value]/text()")[0].strip()
            sub_pri = "$" + item.xpath('.//font[@id]/text()')[0].strip()
...
if __name__ == "__main__":
    # for i in range(1):
    while r.exists("neobiolab_kit_detail"):
        extract = r.rpop("neobiolab_kit_detail")
        # extract = "https://neobiolab.com/index.php?a=show&m=Product&n=Human%20TIMP-3%20ELISA%20Kit|HT0525"
        print(extract)
        try:
            lxml = Neobiolab().format(extract)
        except Exception as e:
            print(e)
            # r.lpush("neobiolab_kit_detail", extract)
            # time.sleep(30)
            print("sleeping...")
            continue
        if lxml is not None:
            brand = Neobiolab().brand()
            kit_type = Neobiolab().kit_type()
            catalog_number = Neobiolab().catalog_number(lxml)
            product_name = Neobiolab().product_name(lxml)
            detail_url = Neobiolab().detail_url(extract)
            tests = Neobiolab().tests(lxml)
            assay_type = Neobiolab().assay_type(lxml)
            detection_method = Neobiolab().detection_method(lxml)
            sample_type = Neobiolab().sample_type(lxml)
            assay_length = Neobiolab().assay_length(lxml)
        #     sensitivity = Neobiolab().sensitivity(lxml)
        #     assay_range = Neobiolab().assay_range(lxml)
        #     specificity = Neobiolab().specificity(lxml)
        #     target_protein = Neobiolab().target_protein(lxml)
        #     geneid = Neobiolab().geneid(lxml)
        #     swissprot = Neobiolab().swissprot(lxml)
            datasheet_url = Neobiolab().datasheet_url(lxml)
            review = Neobiolab().review(lxml)
            image_qty = Neobiolab().image_qty(lxml)
            citations = Neobiolab().citations(lxml)
        #     synonyms = Neobiolab().synonyms(lxml)
            conjugate = Neobiolab().conjugate(lxml)
            species_reactivity = Neobiolab().species_reactivity(lxml)
        #     note = Neobiolab().note(lxml)

        #     sub_citations = Neobiolab().sub_citations(lxml)
            sub_images = Neobiolab().sub_images(image_qty, lxml)
            sub_price = Neobiolab().sub_price(lxml)
            # print(sub_price)
        ...
        if sub_images:
            ...
            for sub in sub_images:
                ...
                new_images = Images(
                    Catalog_Number=catalog_number, Image_url=img)
